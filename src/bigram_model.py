...
def bigrams_count_to_probabilities(
    ...
    # TODO
    smoothed = bigram_counts+smooth_factor
...
def calculate_neg_mean_log_likelihood(
    ...
    # TODO
    total_log_likelihood: torch.tensor = torch.empty(len(words))

    # Calculate the log likelihood for each word and accumulate
    # TODO
    total_log_likelihood = torch.tensor([calculate_log_likelihood(word, bigram_probabilities, char_to_index, start_token, end_token) for word in words])

    # Calculate and return the negative mean log likelihood
    # TODO
    mean_log_likelihood: float = -total_log_likelihood.sum()/len(words)
...
def calculate_log_likelihood(
    ...
    # TODO
    log_likelihood: torch.tensor = torch.empty(len(processed_word)-1)
    ...
    for i in range(len(processed_word)-1):
        ...
        j, k = char_to_index[bigram[0]], char_to_index[bigram[1]]
        log_likelihood[i] = torch.log(bigram_probabilities[j, k])
